...
class CommandInterfaceTests(unittest.TestCase):
    ...
    def test_apple_dashboard_preserves_storage_and_native_fallback(self):
        ...
        self.assertIn(
            'RESOURCE_URL="/local/vps-sentinel-apple-card.js?v=0.9.4"',
            APPLE_DASHBOARD,
        )
    ...
    def test_apple_card_is_self_contained_and_responsive(self):
        ...
        self.assertIn(".insight.pressing { transform: scale(.975); }", APPLE_CARD)
        self.assertIn(".resource:hover,\n        .insight:hover", APPLE_CARD)
        self.assertNotIn("transform: translateY(-1px);", APPLE_CARD)
