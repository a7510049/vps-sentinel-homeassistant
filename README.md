...
0.9 起，Apple 風格面板可選擇加入「主機維護」區，提供檢查更新、
安裝 Ubuntu 安全更新與排程重新啟動。此功能預設關閉，請至
「⚙️ 監控設定 → 遠端維護開關」明確啟用；系統只接受固定操作，
不提供自訂指令或完整系統版本升級。
...
0.9.0 的資源網址為 `/local/vps-sentinel-apple-card.js?v=0.9.0`。
前端內容更新時會同步調整版本參數，避免 Home Assistant App 沿用舊快取。
